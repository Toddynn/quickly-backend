...
const envSchema = object({
     ...
     APP_PUBLIC_PORT: string().optional(),
     ...
     ABACATE_PAY_WEBHOOK_SECRET_KEY: string({ error: 'ABACATE_PAY_WEBHOOK_SECRET_KEY is required.' }),

     ABACATE_PAY_PLAN_SOLO_PRODUCT_ID: string({ error: 'ABACATE_PAY_PLAN_SOLO_PRODUCT_ID is required.' }),
     ABACATE_PAY_PLAN_TEAM_PRODUCT_ID: string({ error: 'ABACATE_PAY_PLAN_TEAM_PRODUCT_ID is required.' }),
     ABACATE_PAY_PLAN_STUDIO_PRODUCT_ID: string({ error: 'ABACATE_PAY_PLAN_STUDIO_PRODUCT_ID is required.' }),

     ABACATE_PAY_PLAN_SOLO_ANNUAL_PRODUCT_ID: string({ error: 'ABACATE_PAY_PLAN_SOLO_ANNUAL_PRODUCT_ID is required.' }),
     ABACATE_PAY_PLAN_TEAM_ANNUAL_PRODUCT_ID: string({ error: 'ABACATE_PAY_PLAN_TEAM_ANNUAL_PRODUCT_ID is required.' }),
     ABACATE_PAY_PLAN_STUDIO_ANNUAL_PRODUCT_ID: string({ error: 'ABACATE_PAY_PLAN_STUDIO_ANNUAL_PRODUCT_ID is required.' }),
});
...
const rawEnv = {
     ...
     APP_PUBLIC_PORT: process.env.APP_PUBLIC_PORT || undefined,
     ...
     ABACATE_PAY_WEBHOOK_SECRET_KEY: process.env.ABACATE_PAY_WEBHOOK_SECRET_KEY,

     ABACATE_PAY_PLAN_SOLO_PRODUCT_ID: process.env.ABACATE_PAY_PLAN_SOLO_PRODUCT_ID,
     ABACATE_PAY_PLAN_TEAM_PRODUCT_ID: process.env.ABACATE_PAY_PLAN_TEAM_PRODUCT_ID,
     ABACATE_PAY_PLAN_STUDIO_PRODUCT_ID: process.env.ABACATE_PAY_PLAN_STUDIO_PRODUCT_ID,

     ABACATE_PAY_PLAN_SOLO_ANNUAL_PRODUCT_ID: process.env.ABACATE_PAY_PLAN_SOLO_ANNUAL_PRODUCT_ID,
     ABACATE_PAY_PLAN_TEAM_ANNUAL_PRODUCT_ID: process.env.ABACATE_PAY_PLAN_TEAM_ANNUAL_PRODUCT_ID,
     ABACATE_PAY_PLAN_STUDIO_ANNUAL_PRODUCT_ID: process.env.ABACATE_PAY_PLAN_STUDIO_ANNUAL_PRODUCT_ID,
} as const;

export const env = envSchema.parse(rawEnv);

const with_port = (protocol: string, domain: string, port?: string) => {
	if (!port) {
	...
	return `${protocol}://${domain.slice(0, pathIndex)}:${port}${domain.slice(pathIndex)}`;
};

export const IS_PRODUCTION = process.env.NODE_ENV === 'production';

export const BACK_END_URL = with_port(env.APP_PROTOCOL, env.APP_DOMAIN, env.APP_PUBLIC_PORT);
