...
import { PROFESSIONAL_WORKING_HOURS_REPOSITORY_KEY } from '../../shared/constants/repository-interface-key';
...
@Injectable()
export class GetProfessionalWorkingHoursUseCase {
	constructor(
		@Inject(PROFESSIONAL_WORKING_HOURS_REPOSITORY_KEY)
		private readonly professionalWorkingHoursRepository: ProfessionalWorkingHoursRepositoryInterface,
	) {}

	async execute(professionalId: string): Promise<WorkingHoursDto[]> {
		const rows = await this.professionalWorkingHoursRepository.find({ where: { professional_id: professionalId } });
